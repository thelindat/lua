...
template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Plane<L, T, Q> &plane, const vec<L, T, Q> &point, T distanceThreshold = contains_eps<T>()) {
  return distance(plane, point) <= distanceThreshold;
}

template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Plane<L, T, Q> &plane, const Line<L, T, Q> &line, const T distanceThreshold = contains_eps<T>()) {
  return contains(plane, line.pos, distanceThreshold) && isPerpendicular(line.dir, plane.normal, distanceThreshold);  // @RemoveSqrt
}

template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Plane<L, T, Q> &plane, const Ray<L, T, Q> &ray, const T distanceThreshold = contains_eps<T>()) {
  return contains(plane, ray.pos, distanceThreshold) && isPerpendicular(ray.dir, plane.normal, distanceThreshold);  // @RemoveSqrt
}

template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Plane<L, T, Q> &plane, const LineSegment<L, T, Q> &line, const T distanceThreshold = contains_eps<T>()) {
  return contains(plane, line.a, distanceThreshold) && contains(plane, line.b, distanceThreshold);
}

template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Plane<L, T, Q> &plane, const Triangle<L, T, Q> &triangle, const T distanceThreshold = contains_eps<T>()) {
...
GLM_GEOM_QUALIFIER bool intersectLinePlane(const vec<L, T, Q> &planeNormal, T planeD, const vec<L, T, Q> &linePos, const vec<L, T, Q> &lineDir, T &d) {
  ...
  d = T(0);
  return epsilonEqual(dot(planeNormal, linePos), planeD, intersect_eps<T>());
}
...
template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool intersects(const Plane<L, T, Q> &plane, const Plane<L, T, Q> &other, Line<L, T, Q> &line) {
