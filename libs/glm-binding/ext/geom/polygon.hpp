...
/// </summary>
template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool diagonalExists(const Polygon<L, T, Q> &polygon, size_t i, size_t j) {
  const size_t size = polygon.size();
...
/// </summary>
template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool isPlanar(const Polygon<L, T, Q> &polygon, const T epsSq = squared_eps<T>()) {
  const size_t size = polygon.size();
  if (size == 0)
    return false;
  else if (size <= 3)
...
/// </summary>
template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool isSimple(const Polygon<L, T, Q> &polygon) {
  GLM_GEOM_ASSUME(isPlanar(polygon), false);
...
/// </summary>
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool isConvex(const Polygon<3, T, Q> &polygon) {
  const size_t size = polygon.size();
  if (size == 0)
    return false;
  else if (size <= 3)
...
template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE vec<L, T, Q> pointOnEdge(const Polygon<L, T, Q> &polygon, const T dist) {
  const size_t size = polygon.size();
  if (size == 0)
    return vec<L, T, Q>(T(0));
  else if (size < 2)
...
/// </summary>
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE Plane<3, T, Q> planeCCW(const Polygon<3, T, Q> &polygon) {
  const size_t size = polygon.size();
  const vec<3, T, Q> hint = unit::forward<T, Q>();
  const vec<3, T, Q> hint2 = unit::up<T, Q>();
  switch (size) {
    case 0: return Plane<3, T, Q>(T(0));
    case 1: return planeFrom(polygon[0], unit::up<T, Q>());
    case 2: {
...
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool contains(const Polygon<3, T, Q> &polygon, const vec<3, T, Q> &worldSpacePoint, PolyContains type, const T thickness = thickness_eps<T>()) {
  const size_t size = polygon.size();
  if (size < 3)
    return false;
  ...
  const vec<3, T, Q> bu = basisU(polygon);
  const vec<3, T, Q> bv = basisV(polygon);
  if (!isNormalized(bu, epsilon<T>()) || !isNormalized(bv, epsilon<T>()) || !isPerpendicular(bu, bv))  // @RemoveSqrt
    return false;
...
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool contains(const Polygon<3, T, Q> &polygon, const vec<3, T, Q> &worldSpace, const T thickness = thickness_eps<T>()) {
  return contains(polygon, worldSpace, PolyContains::Unidirectional, thickness);
}

template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool containsAbove(const Polygon<3, T, Q> &polygon, const vec<3, T, Q> &worldSpace, const T thickness = thickness_eps<T>()) {
  return contains(polygon, worldSpace, PolyContains::Positive, thickness);
}

template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool containsBelow(const Polygon<3, T, Q> &polygon, const vec<3, T, Q> &worldSpace, const T thickness = thickness_eps<T>()) {
  return contains(polygon, worldSpace, PolyContains::Negative, thickness);
}

template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool contains2D(const Polygon<3, T, Q> &polygon, const LineSegment<3, T, Q> &localLineSegment) {
  if (polygon.size() < 3)
    return false;
  ...
  // Segment is fully inside or outside the polygon, determine which.
  return contains(polygon, mapFrom2D(polygon, vec<2, T, Q>(localLineSegment.a.x, localLineSegment.a.y)));
}

template<length_t L, typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Polygon<L, T, Q> &polygon, const Polygon<L, T, Q> &worldSpacePolygon, const T thickness = thickness_eps<T>()) {
  if (polygon.size() == 0)
    return false;
...
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Polygon<3, T, Q> &polygon, const LineSegment<3, T, Q> &worldSpaceLineSegment, const T thickness = thickness_eps<T>()) {
  const size_t size = polygon.size();
  if (size < 3)
    return false;

  const Plane<3, T, Q> plane = planeCCW(polygon);
  if (distance(plane, worldSpaceLineSegment.a) > thickness || distance(plane, worldSpaceLineSegment.b) > thickness)
    return false;
...
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER bool contains(const Polygon<3, T, Q> &polygon, const Triangle<3, T, Q> &worldSpaceTriangle, const T thickness = thickness_eps<T>()) {
...
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool intersects2D(const Polygon<3, T, Q> &polygon, const LineSegment<3, T, Q> &localSpaceLineSegment) {
  const size_t size = polygon.size();
  if (size < 3)
    return false;
...
template<typename T, qualifier Q>
GLM_GEOM_QUALIFIER_OUTINLINE bool intersects(const Polygon<3, T, Q> &polygon, const LineSegment<3, T, Q> &line) {
  const Plane<3, T, Q> plane = planeCCW(polygon);
  const T denom = dot(plane.normal, line.b - line.a);  // Compute line-plane intersection
  if (abs(denom) < intersect_eps<T>()) {  // plane & segment are planar
...
GLM_GEOM_QUALIFIER bool intersects(const Polygon<L, T, Q> &polygon, const Plane<L, T, Q> &plane) {
  ...
  static bool intersectsObject(const Polygon<L, T, Q> &polygon, const Object &obj, const T thickness = thickness_eps<T>()) {
    return false;
  }

  template<length_t L, typename T, qualifier Q>
  GLM_GEOM_QUALIFIER bool intersects(const Polygon<L, T, Q> &polygon, const Polygon<L, T, Q> &other) {
    return intersectsObject(polygon, other);
  }

  template<length_t L, typename T, qualifier Q>
  GLM_GEOM_QUALIFIER bool intersects(const Polygon<L, T, Q> &polygon, const Triangle<L, T, Q> &triangle) {
    return intersectsObject(polygon, triangle);
  }

  template<length_t L, typename T, qualifier Q>
  GLM_GEOM_QUALIFIER bool intersects(const Polygon<L, T, Q> &polygon, const Sphere<L, T, Q> &sphere) {
    return false;
  }
#endif
