...
#define save_and_next(ls) (save(ls, ls->current), ls_next(ls))
...
** is a TValue already available. Later, the code generation can change
** this value.
*/
    ...
    switch (ls->current) {
      ...
      case '-': {  /* '-' or '--' (comment) */
        ...
        while (!currIsNewline(ls) && ls->current != EOZ)
          ls_next(ls);  /* skip until end of line (or end of file) */
        break;
      }
          ...
          if (ls->current == '*') { /* long comment */
            ...
            while (ls->current != EOZ) {
              if (last == '*' && ls->current == '/')
                break;
              last = ls->current;
              ls_next(ls); /* skip until closing marker (or end of file) */
            }
            if (ls->current == EOZ) {
